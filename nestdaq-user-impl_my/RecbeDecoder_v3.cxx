...
int main(int argc, char** argv)
{
    ...
    if(argc == 3){
        ...
        foutputFileName = "/home/dugon/Works/nestdaq/outroot/sample_cosmicray_v3.root"; 
    }
    ...
    struct Data {
        ...
        std::array<std::array<unsigned short, 32>, 48> trig_adc;
        std::array<std::array<unsigned short, 32>, 48> trig_tdc;
        std::array<std::array<unsigned short, 32>, 4992> adc;
        std::array<std::array<unsigned short, 32>, 4992> tdc;
    } data;


    tree.Branch("Packettype", &data.Packettype, "Packettype[105]/b");
    tree.Branch("BoardID", &data.BoardID, "BoardID[105]/b");
    tree.Branch("SentNumber", &data.SentNumber, "SentNumber[105]/s");
    tree.Branch("Trigtime", &data.Trigtime, "Trigtime[105]/s");
    tree.Branch("Datalength", &data.Datalength, "Datalength[105]/s");
    tree.Branch("Upperbit", &data.Upperbit, "Upperbit[105]/s");
    tree.Branch("Lowerbit", &data.Lowerbit, "Lowerbit[105]/s");
    tree.Branch("trig_adc", &data.trig_adc, "trig_adc[48][32]/s");
    tree.Branch("trig_tdc", &data.trig_tdc, "trig_tdc[48][32]/s");
    tree.Branch("adc", &data.adc, "adc[4992][32]/s");
    tree.Branch("tdc", &data.tdc, "tdc[4992][32]/s");
    ...
    while(true){
        ...
        //TFHEADER Part //////////////////////////////////////////////////////////////////////////////////////////
        std::vector<uint8_t> msgTFHeader(sizeof(TF::Header));
        uint64_t magic;
        ...
        for (auto i=0u; i<tfHeader->numSource; ++i) {
            ...
            for(size_t k = 0; k < 6156; k+=2){
                ...
                if(r_BoardID == 1){
                    ...
                    if(AorTdcNum == 1){
                        data.trig_tdc[ChNum][SampleclockNum] = getBitRange<unsigned short>(reinterpret_cast<const char*>(&c_main_data), 0, 14);
                    }else{
                        data.trig_adc[ChNum][SampleclockNum] = c_main_data;
                    }
                    ChNum++;
                }else{
                    if (ChNum == 48){
